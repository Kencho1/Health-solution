...
def search(request):
    ...
    address_hospitals = Gujarat_hospital.objects.filter(address__icontains = query)

    title_Ngo_hospitals = Gujarat_Ngo.objects.filter(title__icontains = query)
    content_Ngo_hospitals = Gujarat_Ngo.objects.filter(content__icontains = query)
    address_Ngo_hospitals = Gujarat_Ngo.objects.filter(address__icontains = query)

    merge_hospital = title_hospitals.union(content_hospitals)
    hospitals = merge_hospital.union(address_hospitals)

    merge_1 = hospitals.union(title_Ngo_hospitals)
    merge_2= merge_1.union(content_Ngo_hospitals)
    merge_final= merge_2.union(address_Ngo_hospitals)

    if len(merge_final)==0:
        messages.warning(request,f'No search results found.Please refine your query')

    context = {'gujarat':merge_final,'query':query}
